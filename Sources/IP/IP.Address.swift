extension IP {
    ...
    public protocol Address: LosslessStringConvertible,
        Comparable,
        Equatable,
        Hashable,
        Sendable {
...
extension IP.Address {
    ...
    @inlinable public var value: Storage { .init(bigEndian: self.storage) }
}
extension IP.Address where Self: ExpressibleByIntegerLiteral,
    Self.IntegerLiteralType == Storage {
    @inlinable public init(integerLiteral: Storage) { self.init(value: integerLiteral) }
}
...
extension IP.Address {
    ...
    @inlinable static var bitWidth: UInt8 { .init(Storage.bitWidth) }
