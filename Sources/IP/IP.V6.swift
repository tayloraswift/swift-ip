extension IP {
    ...
    @frozen public struct V6: Address, ExpressibleByIntegerLiteral {
        ...
        public var storage: UInt128

        /// Creates an IPv6 address by wrapping a raw **big-endian** integer.
        @inlinable public init(storage: UInt128) {
...
extension IP.V6 {
    ...
    /// byte order.
    @inlinable package init(storage: (UInt32, UInt32, UInt32, UInt32)) {
    ...
    @inlinable package init(
        storage: (UInt16, UInt16, UInt16, UInt16, UInt16, UInt16, UInt16, UInt16)
    ) {
        self = withUnsafeBytes(of: storage) { .copy(buffer: $0) }
    }
...
extension IP.V6 {
    ...
    @inlinable public init(v4: IP.V4) {
...
extension IP.V6 {
    ...
    @inlinable public static func copy(
        from bytes: some RandomAccessCollection<UInt8>
    ) -> Self? {
        bytes.count == MemoryLayout<UInt128>.size ? .copy(buffer: bytes) : nil
    }

    @inlinable static func copy(buffer: some RandomAccessCollection<UInt8>) -> Self {
...
extension IP.V6 {
    ...
    @inlinable public static var zero: Self { .init(storage: 0) }

    /// Returns the all-ones address, `ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff`.
    @inlinable public static var ones: Self { .init(storage: ~0) }

    /// Returns the loopback address, `::1`. There is only one loopback address in IPv6.
    @inlinable public static var localhost: Self { .init(value: 1) }
}
extension IP.V6: RandomAccessCollection {
    ...
    @inlinable public var endIndex: Int { 8 }

    /// Returns the *i*th word of the address, in **platform** byte order.
    @inlinable public subscript(i: Int) -> UInt16 {
        get {
...
extension IP.V6: LosslessStringConvertible {
    ...
    @inlinable public init?(_ string: some StringProtocol) {
        ...
        for w: Int in 0 ..< 8 {
            guard
            let j: String.Index = string[i...].firstIndex(of: ":") else {
                guard w == 7,
                let word: UInt16 = .init(string[i...], radix: 16) else {
                    return nil
                }
            ...
            guard
            let word: UInt16 = .init(string[i ..< j], radix: 16) else {
                return nil
            }
        ...
        for w: Int in (z ..< 8).reversed() {
            guard
            let j: String.Index = string[i ... k].lastIndex(of: ":") else {
                return nil
            }
            ...
            guard
            let word: UInt16 = .init(string[string.index(after: j) ... k], radix: 16) else {
                return nil
            }
