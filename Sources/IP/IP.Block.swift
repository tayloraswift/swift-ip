extension IP {
    ...
    @frozen public struct Block<Base>: Equatable, Hashable, Sendable where Base: Address {
        ...
        @inlinable init(base: Base, bits: UInt8) {
            self.base = base
            self.bits = bits
        }
    }
...
extension IP.Block<IP.V6> {
    ...
    @inlinable public init(v4: IP.Block<IP.V4>) {
        self.init(base: .init(v4: v4.base), bits: v4.bits + 96)
    }

    /// Returns the IPv6 loopback mask, `::1/128`.
    @inlinable public static var loopback: IP.Block<IP.V6> {
        .init(base: .localhost, bits: 128)
    }
}
extension IP.Block<IP.V4> {
    /// Returns the IPv4 loopback mask, `127.0.0.0/8`.
    @inlinable public static var loopback: IP.Block<IP.V4> { .init(base: .localhost, bits: 8) }
}
extension IP.Block {
...
extension IP.Block: CustomStringConvertible {
    ...
    @inlinable public var description: String { "\(self.base)/\(self.bits)" }
}
