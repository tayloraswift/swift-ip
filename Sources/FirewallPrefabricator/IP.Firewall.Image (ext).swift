...
extension IP.Firewall.Image {
    static func build(from ranges: [IPinfo.ASNRange]) throws -> Self {
        ...
        var image: Self = .init(
            autonomousSystems: table.values.sorted { $0.number < $1.number }
        )
    ...
    mutating func colorByCountry(from ranges: [IPinfo.CountryRange]) throws {
    ...
    mutating func colorByClaimant(_ claims: [IP.Claims]) throws {
