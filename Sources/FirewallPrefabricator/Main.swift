...
extension Main {
    func run() throws {
        var image: IP.Firewall.Image = try .build(
            from: try .splitting(
                try .init(
        ...
        let googlebot: SearchbotWhitelist = try .decode(
            parsing: try .init(
        ...
        let bingbot: SearchbotWhitelist = try .decode(
            parsing: try .init(
        ...
        let github: GitHubWhitelist = try .decode(
            parsing: try .init(
