...
extension IP {
    ...
    /// Number (ASN).
    @frozen public struct ASN: Equatable, Hashable, Sendable {
        public let value: UInt32

        @inlinable public init(value: UInt32) {
            self.value = value
        }
    }
}
extension IP.ASN: ExpressibleByIntegerLiteral {
    @inlinable public init(integerLiteral: UInt32) { self.init(value: integerLiteral) }
}
...
extension IP.ASN: LosslessStringConvertible {
    @inlinable public init?(_ string: some StringProtocol) {
        guard
        let value: UInt32 = .init(string) else {
            return nil
        }
...
extension IP.ASN: BSON.BinaryPackable {
    @inlinable public static func get(_ storage: UInt32) -> Self { .init(value: .get(storage)) }

    @inlinable public consuming func set() -> UInt32 { self.value.set() }
}
