...
extension IP.Table {
    @frozen @usableFromInline struct SortedArray<Bound> where Bound: Comparable,
        Bound: BSON.BinaryPackable {
        @usableFromInline let ranges: BSON.BinaryArray<ClosedRange<Bound>>
        @usableFromInline let colors: BSON.BinaryArray<Color>
...
extension IP.Table.SortedArray where Bound: Sendable {
    init(
        ...
        guard ranges.count == colors.count else {
...
extension IP.Table.SortedArray {
    @inlinable func color(containing ip: Bound) -> Color? {
    ...
    @inlinable func index(containing ip: Bound) -> Int {
