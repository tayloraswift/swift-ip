...
extension IP {
    ...
    @frozen public struct AS: Hashable, Equatable, Sendable {
        public let number: ASN
        public var domain: String
        public var name: String

        @inlinable public init(number: ASN, domain: String, name: String) {
            self.number = number
            self.domain = domain
            self.name = name
        }
...
extension IP.AS {
    @inlinable public init(number: IP.ASN, metadata: Metadata) {
        self.init(number: number, domain: metadata.domain, name: metadata.name)
    }

    @inlinable public var metadata: Metadata { .init(domain: self.domain, name: self.name) }
}
extension IP.AS {
