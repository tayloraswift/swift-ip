...
extension IP {
    @frozen public struct Firewall: Sendable {
        private let autonomousSystems: [ASN: AS.Metadata]

        public let asn: Table<ASN>
        public let country: Table<ISO.Country>

        private let claimant: Table<Int32>
        private let claimants: [Claimant]
        private let loopback: Claimant?
...
extension IP.Firewall {
    public static func load(from image: Image, loopback: IP.Claimant? = nil) -> Self {
    ...
    public func lookup(v4 ip: IP.V4) -> (IP.AS?, IP.Claimant?) {
        let asn: IP.ASN? = self.asn[v4: ip]
        let claimant: Int32? = self.claimant[v4: ip]
        return self.symbolicate(
            asn: asn,
            claimant: claimant,
            loopback: IP.Block<IP.V4>.loopback.contains(ip)
        )
    }
    public func lookup(v6 ip: IP.V6) -> (IP.AS?, IP.Claimant?) {
